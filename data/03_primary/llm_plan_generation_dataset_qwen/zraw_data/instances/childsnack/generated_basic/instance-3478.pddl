; child-snack task with 2 children and 0.63 gluten factor 
; constant factor of 1.7
; random seed: 891280

(define (problem prob-snack)
  (:domain child-snack)
  (:objects
    child390 child58 - child
    bread372 bread331 - bread-portion
    content147 content222 - content-portion
    tray95 tray121 - tray
    table58 table264 table282 - place
    sandw1 sandw2 sandw3 sandw4 - sandwich
  )
  (:init
     (at tray95 kitchen)
     (at tray121 kitchen)
     (at_kitchen_bread bread372)
     (at_kitchen_bread bread331)
     (at_kitchen_content content147)
     (at_kitchen_content content222)
     (no_gluten_bread bread331)
     (no_gluten_content content147)
     (allergic_gluten child58)
     (not_allergic_gluten child390)
     (waiting child390 table264)
     (waiting child58 table282)
     (notexist sandw1)
     (notexist sandw2)
     (notexist sandw3)
     (notexist sandw4)
  )
  (:goal
    (and
     (served child390)
     (served child58)
    )
  )
)
